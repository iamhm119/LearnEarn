...
const ResultScreen = ({ result, onRetake, onNext, navigate }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto animate-slide-up">
      {/* Score card */}
      <div className={`premium-card text-center mb-8 border-2 ${passed ? "border-success-200 bg-success-50/30" : "border-danger-200 bg-danger-50/30"}`}>
        <div className={`w-28 h-28 rounded-full mx-auto mb-5 flex items-center justify-center text-4xl font-black shadow-inner
          ${passed ? "bg-success-100 text-success-600" : "bg-danger-100 text-danger-600"}`}>
          {percentage}%
        </div>
        <h2 className={`text-3xl font-bold mb-2 ${passed ? "text-success-600" : "text-danger-600"}`}>
        ...
        <div className="flex gap-4 justify-center flex-wrap">
          ...
          {passed && <button onClick={onNext} className="btn-primary flex items-center gap-2 shadow-elevated">
      ...
      <div className="space-y-3">
        {results.map((r, i) => (
          <div key={i} className={`card p-4 border transition-colors ${r.isCorrect ? "border-success-200 bg-success-50/50" : "border-danger-200 bg-danger-50/50"}`}>
            <div className="flex items-start gap-4">
              {r.isCorrect ? (
                <div className="bg-success-100 p-1 rounded-full"><CheckCircle2 size={20} className="text-success-600 flex-shrink-0" /></div>
              ) : (
                <div className="bg-danger-100 p-1 rounded-full"><XCircle size={20} className="text-danger-600 flex-shrink-0" /></div>
              )}
              <div>
    ...
    <div className="min-h-screen bg-surface-50 relative">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 pt-24 pb-12">
        ...
        {result ? (
          ...
          <>
            {/* Quiz header */}
            <div className="card mb-6 animate-slide-up shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h1 className="text-xl font-bold text-txt-primary">Module Quiz</h1>
                  <p className="text-sm font-medium text-txt-secondary">{questions.length} questions — Score ≥50% to pass</p>
                </div>
                <div className="text-right bg-surface-50 px-4 py-2 rounded-xl border border-surface-200">
            ...
            {/* Question */}
            <div className="card mb-6 animate-fade-in shadow-sm" key={current}>
              ...
              <div className="space-y-3">
                {questions[current].options.map((option, optIdx) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`inline-flex items-center justify-center w-7 h-7 rounded-lg text-sm font-bold mr-4 border shadow-sm
                        ${selected ? "bg-brand-600 text-white border-brand-700" : "bg-surface-50 text-txt-secondary border-surface-200"}`}>
            ...
            {/* Navigation controls */}
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6 bg-white p-4 rounded-2xl shadow-sm border border-surface-200">
              ...
              <div className="flex gap-2 flex-wrap justify-center">
                {questions.map((_, i) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-lg text-xs font-bold transition-all duration-200 border shadow-sm
                      ${i === current ? "bg-brand-600 text-white border-brand-700 scale-110" : answers[i] !== null ? "bg-success-50 text-success-600 border-success-200" : "bg-white text-txt-secondary border-surface-200 hover:border-brand-300"}`}
                  >
                    {i + 1}
                  </button>
                ))}
              ...
              {current < questions.length - 1 ? (
                ...
                <button
                  ...
                  className="btn-primary flex items-center gap-2 disabled:opacity-50 px-6 shadow-elevated"
                >
                  {submitting ? (
